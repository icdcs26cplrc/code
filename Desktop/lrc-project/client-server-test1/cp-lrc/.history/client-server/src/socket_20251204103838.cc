...
std::mutex Socket::output_mutex_;
Socket::Socket()
{
}
...
void Socket::sendData(const char *buf, size_t chunk_size, size_t packet_size, const char *des_ip, int des_port_num)
{
  ...
  const char *denormalized_ip = des_ip;
  ...
  cout << "finish send data ! " << endl; // sent_len<< endl;
  close(client_socket);
}
 ...
 */
void Socket::paraRecvData(int server_port_num, char *total_recv_data, size_t chunk_size, size_t packet_size)
{ // int num_conn, int* mark_recv, int flag, char** source_IPs){
  ...
  int connfd = accept(server_socket, (struct sockaddr *)&remote_addr, &length);
  ...
  //}
  recvData(connfd, total_recv_data, chunk_size, packet_size); // index);//, mark_recv);
...
bool Socket::SendDataReliably(int sock_fd, const void *data, size_t data_size)
{
  ssize_t total_sent = 0;
  ssize_t remaining = data_size;
  ...
  {
    ssize_t bytes_sent = write(sock_fd, data_ptr + total_sent, remaining);
    ...
    if (bytes_sent < 0)
    {
      if (errno == EAGAIN || errno == EWOULDBLOCK)
      {
        ...
        {
          std::cerr << "Max retries reached for socket write" << std::endl;
          return false;
        }
        cout << "Socket buffer full, retrying... (" << retry_count << "/" << max_retries << ")" << endl;
        usleep(1000 * retry_count); // 逐渐增加等待时间
        continue;
      }
      else if (errno == EPIPE || errno == ECONNRESET)
      {
        std::cerr << "Connection broken during send: " << strerror(errno) << std::endl;
        return false;
      }
      else
      {
    ...
    {
      std::cerr << "Warning: write() returned 0, connection might be closed" << std::endl;
      return false;
    }
    ...
    retry_count = 0; // 重置重试计数
                     // cout << "Sent chunk: " << bytes_sent << " bytes, total: " << total_sent << "/" << data_size << endl;
  }
  std::lock_guard<std::mutex> log_lock(output_mutex_);
  cout << "Successfully sent all " << total_sent << " bytes" << endl;
  return true;
}

bool Socket::ReceiveDataReliably(int sock_fd, void *buffer, size_t data_size)
{
  ...
  while (total_received < (ssize_t)data_size)
  {
    ssize_t bytes_received = read(sock_fd, buffer_ptr + total_received, remaining);

    if (bytes_received == 0)
    {
      // 连接正常关闭
      cout << "Connection closed by peer (received " << total_received << "/" << data_size << " bytes)" << endl;
      return false;
    }
    else if (bytes_received < 0)
    {
      if (errno == EAGAIN || errno == EWOULDBLOCK)
      {
        ...
        if (retry_count >= max_retries)
        {
          std::cerr << "Max retries reached for socket read" << std::endl;
          return false;
        }
        cout << "No data available, retrying... (" << retry_count << "/" << max_retries << ")" << endl;
    ...
    retry_count = 0; // 重置重试计数
                     // cout << "Received chunk: " << bytes_received << " bytes, total: " << total_received << "/" << data_size << endl;
  }
  std::lock_guard<std::mutex> log_lock(output_mutex_);
  // cout << "Successfully received all " << total_received << " bytes" << endl;
  return true;
}
