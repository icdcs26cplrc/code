...
int UNBALANCED_LRC::single_decode_node_need(int fail_one, int *&node_id, int group_id__)
{
...
bool UNBALANCED_LRC::single_decode(int fail_one, char **data_ptrs, char **code_ptr, size_t decode_size)
{
    ...
    if (fail_one >= (k_ + r_ - 1) && fail_one < (k_ + r_ + p_))
    {
        ...
        decode = jerasure_matrix_decode(p_, 1, 8, final_matrix, 1, erasures, data_ptrs, code_ptr, decode_size);
    }
    else
    {
        ...
        if (fail_one < (k_ + r_ - 1 - group_size_))
        {
            ...
            for (int i = start; i < start + group_real; i++)
            {
                ...
                final_matrix[(i - start)] = local_parity[group_id[0] * k_ + i];
            }
        ...
        else
        {
            ...
            for (int i = 0; i < group_size_; i++)
            {
                if (i >= (group_size_ - (r_ - 1)) && i < group_size_)
                {
                    ...
                    final_matrix[i] = beta[i - (group_size_ - (r_ - 1))];
                }
                else
                {
                    // final_matrix[group_real * group_real + i] = alpha[i];
                    final_matrix[i] = alpha[i];
                }
            }
            decode = jerasure_matrix_decode(group_size_, 1, 8, final_matrix, 0, erasures, data_ptrs, code_ptr, decode_size);
        }
    }
    if (decode == 0)
    {
...
bool UNBALANCED_LRC::muti_single_decode(int fail_num, int *fail_list, char **data_ptrs, char **global_code_ptr, char **local_ptr, size_t decode_size)
{
    int *group_id;
    ...
    if (top_fail == 1)
    {
        ...
        bool top = single_decode(top_level[0], data_p, code_p, decode_size);
        if (top == false)
        {
    ...
    if (fail_num > 0)
    {
        ...
        for (int i = 0; i < p_; i++)
        {
            ...
            if (fail_list_group_id[i] == 1 && (top_fail == 0 || top_fail > 1))
            {
                ...
                bool repair = single_decode(fail_list[temp[0]], data_ptr_, local_r, decode_size);
                if (repair == false)
                {
        ...
        if (need_second_decode && !take_out_global)
        {
            ...
            bool top = single_decode(fail_list[top_fail_id[1]], data_p_sec, code_p_sec, decode_size);
            if (top == false)
            {
            ...
            bool repair = single_decode(fail_list[top_fail_id[0]], data_ptr_sec, local_ptr + group_id[top_fail_id[0]], decode_size);
            if (repair == false)
            {
        ...
        else if (take_out_global)
        {
            ...
            char **code_p_sec = new char *[1];
            code_p_sec[0] = global_code_ptr[r_ - 1];
            bool top = single_decode((k_ + r_ - 1), data_p_sec, code_p_sec, decode_size);
            if (top == false)
            {
...
bool UNBALANCED_LRC::if_need_mds(int fail_num, int *fail_list, int *fail_list_group_id)
{
    ...
    else
    {
        ...
        for (int j = 0; j < 2 * p_; j++)
        {
            ...
            if ((group_fail_num[j] > 1 && j < p_) || count_top_level > 1 || (take_out_global && count_top_level > 0))
            {
                cout << "need mds" << endl;
                cout << "group_fail_num[" << j << "] = " << group_fail_num[j] << endl;
...
void UNBALANCED_LRC::get_group_id(int fail_num, int *fail_list, int *&group_id)
{
    ...
    else
    {
        ...
        for (int i = 0; i < fail_num; i++)
        {
            if (fail_list[i] >= 0 && fail_list[i] < small_part)
            {
...
int UNBALANCED_LRC::muti_decode_node_need(int fail_number, int *fail_list, int *&node_id)
{
...
int UNBALANCED_LRC::muti_top_decode_node_need(int top_fail, int fail_num, int *fail_list, int *&node_id, int *fail_group_id)
{
    int repair_num = get_repair_num(fail_num, fail_group_id);
    // 处理top层的fail
    // 处理top层第一个fail &&global fail or local fail(从后往前)
    ...
    {
        int top_fail_first = single_decode_node_need_(fail_list[fail_num - 1], node_need_first, fail_group_id[fail_num - 1]);
    }
...
int UNBALANCED_LRC::single_decode_node_need_(int fail_one, int *&node_id, int group_id)
{
    int return_group_size;
    // top层出错(k+r-1 - k+r+p-1)
    if (group_id >= p_)
    ...
    else
    {
        ...
        {
            if (group_id < (p_ - remainder_))
            {
                node_id = new int[group_size_ - 1];
                start = group_id * (group_size_ - 1);
                end = start + group_size_ - 1;
                return_group_size = group_size_ - 1;
            }
            else
            {
                node_id = new int[group_size_];
                start = (p_ - remainder_) * (group_size_ - 1) + (group_id - (p_ - remainder_)) * group_size_;
                end = start + group_size_;
                return_group_size = group_size_;
            }
        }
