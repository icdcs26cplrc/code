...
//         if (!port_strings.empty()) {
//             // 使用配置文件中的端口列表
//             for (const auto& port_str : port_strings) {
//                 try {
//                     datanode_ports.push_back(std::stoi(port_str));
//                 } catch (const std::exception& e) {
//                     // 解析失败时使用默认值
//                     datanode_ports.push_back(34567 + static_cast<int>(datanode_ports.size()));
//                 }
//             }
//         } else {
//             // 如果没有配置端口列表，使用基础端口生成
...
//         // 确保至少有15个端口
//         while (datanode_ports.size() < 8) {
//             int base_port = datanode_ports.empty() ? 34567 : datanode_ports.back() + 1;
//             datanode_ports.push_back(base_port);
//         }
//         initialized = true; 
//     }
...
// // 根据node_id获取数据节点IP (node_id / 15)
// inline const char* getDatanodeIPByNodeId(int node_id) {
//     const auto& ips = getDatanodeIPs();
//     int ip_index = node_id / 15;
...
// // 根据node_id获取数据节点端口 (node_id % 15)
// inline int getDatanodePortByNodeId(int node_id) {
//     const auto& ports = getDatanodePorts();
//     int port_index = node_id % 15;
...
//     // 如果超出范围，使用模运算循环
//     if (!ports.empty()) {
//         return ports[port_index % ports.size()];
//     }
...
inline int getDatanodePortByNodeId(int global_node_id) {
    const auto& ports = getDatanodePorts();
    int port_index = global_node_id % DATANODES_PER_IP;  // 在8个端口中循环
...
#define DATANODE_IP_0 getDatanodeIP(0)
#define DATANODE_IP_1 getDatanodeIP(1)
#define DATANODE_IP_2 getDatanodeIP(2)
#define DATANODE_IP_3 getDatanodeIP(3)
#define DATANODE_IP_4 getDatanodeIP(4)
#define DATANODE_IP_5 getDatanodeIP(5)
#define DATANODE_IP_6 getDatanodeIP(6)
#define DATANODE_IP_7 getDatanodeIP(7)
#define DATANODE_IP_8 getDatanodeIP(8)
#define DATANODE_IP_9 getDatanodeIP(9)
#define DATANODE_IP_10 getDatanodeIP(10)
#define DATANODE_IP_11 getDatanodeIP(11)
#define DATANODE_IP_12 getDatanodeIP(12)
#define DATANODE_IP_13 getDatanodeIP(13)
#define DATANODE_IP_14 getDatanodeIP(14)
