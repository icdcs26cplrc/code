...
void Proxy::ProcessGet_Proxy(std::string value)
{
    ...
    else if (command == "datanode")
    {
        ...
        for (int i = 0; i < num; i++)
        {
            threads.emplace_back([this, i, node_id, offset, value_len_]()
                                 { SendGetRequest(node_id[i], offset[i], value_len_[i]); });
        }
        // Wait for all threads to complete
        for (auto &thread : threads)
        {
...
void Proxy::SendGetRequest(int node_id, int offset, size_t value_len)
{
...
void Proxy::SendBlockToDatanode(int datanode_port, int node_id, const char *data, size_t block_size)
{
    ...
    if (bytes_sent < 0) {
        ...
        close(sock_data);
        return;
    } else if (bytes_sent != (ssize_t)buf_str.size()) {
        std::cerr << "Warning: Partial data sent (" << bytes_sent << "/" << buf_str.size() << " bytes)" << std::endl;
    }
...
void Proxy::SendMassgeToD(string destination, string value)
{
    ...
    if (!SendDataReliably(sock, value.c_str(), chunk_size)) {
