...
void LRC_BASE::encode(char **data_ptrs, char **global_coding_ptrs, char **local_coding_ptrs)
{
    ...
    for (int i = 0; i < (k_val+r_val+p_val); i++)
    {
       if (i<(k_val))
       {
        cout<<" encode : size of data_ptr :"<<strlen(data_ptrs[i])<<endl;
       }else if (i>=k_val && i < (k_val+r_val))
       {
        cout<<" encode : size of global_ptr :"<<strlen(global_coding_ptrs[i-k_val])<<endl;
       }else if(i >= (k_val+r_val) && i< (k_val+r_val+p_val)){
        cout<<" encode : size of data_ptr :"<<strlen(local_coding_ptrs[i-k_val-r_val])<<endl;
       }
...
bool LRC_BASE::mds_decode(int locality, int fail_num, int *fail_list, char **data_ptrs, char **code_ptr)
{
    ...
    // debug
     for (int i = 0; i < (k_val+r_val); i++)
     {
         if (i< k_val)
         {
             cout << "mds decode : data_ptrs["<<i<<"] = " << string(data_ptrs[i]).size() << endl;
         }else{
             cout << "mds decode : code_ptr["<<i-k_val<<"] = " << string(code_ptr[i-k_val]).size() << endl;
         }
...
int LRC_BASE::muti_decode_node_need(int fail_number, int *fail_list, int *&node_id)
{
    ...
    if (flag != -1)
    {
        if (if_mds == false)
        {
            ...
            //可能存在重复节点故+10
            node_id = new int[repair_num+10];
            int node_need_num = 0;
            int node_need = 0;
            for (int i = 0; i < fail_number && node_need_num < (repair_num+10); i++)
            {
