...
void LRC_BASE::encode(char **data_ptrs, char **global_coding_ptrs, char **local_coding_ptrs)
{
    ...
    // }
    // 检查编码是否有效
    int *erased = new int[0];
    erased[0]=1;
    erased[1]=-1;
    int decode =jerasure_matrix_decode(k_val, 1, 8, &local_matrix[0], 0, erased, data_ptrs, &local_coding_ptrs[0], blocksize_val);
...
bool LRC_BASE::mds_decode(int locality, int fail_num, int *fail_list, char **data_ptrs, char **code_ptr,  size_t decode_size )
{
    ...
    int *final_matrix = new int[( r_val + locality) * k_val];
    ...
    // for (int i = 0; i < (k_val + r_val + locality); i++)
    // {
    //     for (int j = 0; j < k_val; j++)
    //     {
    //         cout << "final_matrix[" << i * k_val + j << "] = " << final_matrix[i * k_val + j] << endl;
    ...
    char **global_code_ptr = new char *[r_val];
    for (int i = 0; i < r_val; i++)
    {
...
if (decode_size == 0)
{
    decode_size = blocksize_val;
}else{
    decode_size = decode_size;
}

    int decode = jerasure_matrix_decode(k_val, r_val + locality, 8, final_matrix, 0, erasures, data_ptrs, code_ptr, decode_size);
//     for (int i = 0; i < (k_val+r_val); i++)
//     {
//         if (i< k_val)
//         {
