...
void Proxy::ProcessGet_Proxy(std::string value)
{
    ...
    else if (command == "datanode")
    {
        ...
        else
        {
            ...
            size_t *fail_value_len_ = new size_t[fail_num];
            ...
            for (int i = 0; i < fail_num; i++)
            {
                fail_offset[i] = offset[error_id[i]];
                fail_value_len_[i] = max_fail_len; // 使用最大值作为降级读的长度
            }
            if (repair_message != nullptr && *actual_length > 0)
            {
                cout << "Received repair message, starting degraded read process" << endl;
                // 执行降级读修复
                Repair_degrade(repair_message, num, node_id, fail_offset, fail_value_len_,offset, value_len_, value_get_to_client);
...
void Proxy::Repair_degrade(string message, int value_get_num, int *node_id, const size_t *fail_offset,const int*fail_value_len_, const int *offset_value, size_t *repair_value_len, string &value)
{
    // parse value
    ...
    std::vector<size_t> repair_value_len_vec(fail_value_len_, fail_value_len_ + num);
