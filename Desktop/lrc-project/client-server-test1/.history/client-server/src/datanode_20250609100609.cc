...
int main()
{
    ...
    for (int i = 0; i < datanode_number; i++) {
        ...
        if (pid == 0) {
            // 子进程：运行单个DataNode
            StartSingleDataNode(i, k, r, p, block_size);
            exit(0); // 子进程不应该到达这里
        }
        else if (pid > 0) {
            // 父进程：记录子进程PID
    ...
    signal(SIGINT, [&child_pids](int sig) {
        std::cout << "Received SIGINT, terminating all DataNode processes..." << std::endl;
        for (pid_t child_pid : child_pids) {
            kill(child_pid, SIGTERM);
        }
        exit(0);
    ...
    signal(SIGTERM, [&child_pids](int sig) {
        std::cout << "Received SIGTERM, terminating all DataNode processes..." << std::endl;
        for (pid_t child_pid : child_pids) {
            kill(child_pid, SIGTERM);
        }
        exit(0);
    ...
    while (!child_pids.empty()) {
        ...
        if (finished_pid > 0) {
            // 移除已结束的进程
            auto it = std::find(child_pids.begin(), child_pids.end(), finished_pid);
