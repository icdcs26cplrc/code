...
void DataNode::Start(int port, const std::string& bind_ip ) {
    ...
    // 如果指定了绑定IP，使用指定IP，否则使用配置的IP
    std::string target_ip;
    if (!bind_ip.empty()) {
        target_ip = bind_ip;
    } else {
        // 根据端口计算IP地址（保持原有逻辑的兼容性）
...
//     cout<<"Please input the number of datanodes,the number of global parity,the number of local parity,blocksize "<<endl;
...
// }
// 单个DataNode进程的启动函数
// void DataNode::StartSingleDataNode(int node_index, int k, int r, int p, size_t block_size) {
//     try {
//         auto datanode = std::make_unique<DataNode>();
//         int dataport = DATANODE_PORT_BASE + node_index;
//         int datanode_number = k + r + p;

//         datanode->block_size = block_size;
//         datanode->SetNodeList(datanode_number);

//         std::cout << "Starting DataNode " << node_index << " on port " << dataport << std::endl;
//         datanode->Start(dataport);
//     }
//     catch (const std::exception& e) {
//         std::cerr << "DataNode " << node_index << " failed: " << e.what() << std::endl;
//         exit(1);
//     }
// }

void DataNode::StartSingleDataNode(int global_node_id, int ip_index, int local_port_index, int k, int r, int p, size_t block_size)
{
    try
    {
        auto datanode = std::make_unique<DataNode>();
        ...
        // 获取IP和端口
        std::string target_ip = getDatanodeIPByNodeId(global_node_id);
        int target_port = getDatanodePortByNodeId(global_node_id);
        ...
        std::cout << "Starting DataNode " << global_node_id
                  << " on " << target_ip << ":" << target_port << std::endl;

        // 使用新的Start方法
        datanode->Start(target_port, target_ip);
    }
    catch (const std::exception &e)
    {
...
//     {
//         pid_t pid = fork();
//         DataNode datanode; // 每个子进程都有自己的DataNode实例
//         if (pid < 0)
//         {
//             // fork失败
//             perror("fork failed");
...
//         {
//             // 子进程：运行单个DataNode
//             datanode.StartSingleDataNode(i, k, r, p, block_size);
//             exit(0); // 子进程不应该到达这里
//         }
//         else if (pid > 0)
//         {
...
//             std::cout << "Created DataNode process " << i << " with PID " << pid << std::endl;
//         }
//         else
//         {
//             // fork失败
...
//     // 父进程：监控子进程
//     std::cout << "All DataNode processes started. Parent process monitoring..." << std::endl;

//     // 设置信号处理，当收到SIGINT或SIGTERM时，终止所有子进程
//     // signal(SIGINT, signal_handler);
//     // signal(SIGTERM, signal_handler);

//     // 等待所有子进程结束并监控它们的状态
//     while (!g_child_pids.empty())
//     {
//         int status;
...
//         {
//             // 移除已结束的进程
//             auto it = std::find(g_child_pids.begin(), g_child_pids.end(), finished_pid);
//             if (it != g_child_pids.end())
//             {
//                 if (WIFEXITED(status))
//                 {
//                     std::cout << "DataNode process " << finished_pid << " exited with status " << WEXITSTATUS(status) << std::endl;
//                 }
//                 else if (WIFSIGNALED(status))
//                 {
//                     std::cout << "DataNode process " << finished_pid << " was killed by signal " << WTERMSIG(status) << std::endl;
//                 }
//                 g_child_pids.erase(it);
//             }
//         }
//     }
