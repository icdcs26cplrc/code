...
void Proxy::Repair_degrade(string message, int value_get_num, const int *node_id, const int *offset_value, const size_t *repair_value_len, string &value, char **data_alredy_get)
{
    ...
    // {
    //     total_fail_value_len_+= repair_value_len[fail_node[i]];
    // }
    // // dont need changing
    // if (total_fail_value_len_ >= blocksize)
    // {
...
void Proxy::compute_degrade_offsets_and_lens(const int node_need_num, const int fail_num, const size_t block_size,
                                             const std::vector<int> &first_get_node_ids, const std::vector<int> &fail_node_ids, const std::vector<int> &degrade_get_node_ids,
                                             ...
                                             std::vector<int> &offset_vec, std::vector<size_t> &value_len_vec)
{
    ...
    {
        offset_vec.resize(node_need_num, 0);
        value_len_vec.resize(node_need_num, block_size);
    }
    else
    {
        // 故障发生在头部或尾部
        if (fail_num == 1)
        {
            ...
            for (int i = 0; i < degrade_get_num; i++)
            {
                ...
                { // 存在重复读的可能
                    ...
                    {
                        size_t value_end_pos = offsets[first_get_node_ids[j] - first_get_node_ids[0]] + value_lens[first_get_node_ids[j] - first_get_node_ids[0]];
                        size_t value_start_pos = offsets[first_get_node_ids[j] - first_get_node_ids[0]];
