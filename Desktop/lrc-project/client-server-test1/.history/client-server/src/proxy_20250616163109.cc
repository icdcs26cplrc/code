    ...
    else if (command == "datanode")
    {
        ...
        else
        {
            ...
            if (repair_message != nullptr && *actual_length > 0)
            {
                ...
                // 执行降级读修复
                Repair_degrade(repair_message, num, node_id, min_offset, max_fail_len,offset, value_len_, value_get_to_client);
...
void Proxy::Repair_degrade(string message, int value_get_num, int *node_id, const int fail_offset, size_t fail_value_len_, const int *offset_value, size_t *repair_value_len, string &value)
{
    // parse value
