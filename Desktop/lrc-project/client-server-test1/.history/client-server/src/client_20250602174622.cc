...
bool Client::get(std::string key, std::string &value)
{
  ...
  if(inet_aton(denormalized_ip, &remote_addr.sin_addr) == 0){
    ...
    {
        // Successfully accepted a connection, process the request
        // 后面需要修改为blocksize
        ...
        {
            ssize_t received = read(client_sock, buffer, block_size);
            if (received == 0)
            {
                std::cerr << " received " << total_received << endl; //"/" << block_size << " bytes)" << std::endl;
                break;
            }
            else if (received < 0)
            {
