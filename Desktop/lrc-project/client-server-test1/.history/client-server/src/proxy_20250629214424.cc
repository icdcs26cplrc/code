...
void Proxy::Repair_degrade(string message, int value_get_num, const int *node_id, const int *offset_value, const size_t *repair_value_len, string &value, char **data_alredy_get)
{
        ...
        if (!is_failed && pos < num && i == node_need_id[pos])
        {
            ...
            if (recv_len[pos] < afer_computer_fail_value_len)
            {
                ...
                //错误发生在头部
                  if (symbols == 11)
                  {
                    temp = std::string(data_alredy_get[i - node_id[0]] + offset_value[fail_node[0] - node_id[0]]);
                    temp.append(recv_data[pos]);
                    // Free the old memory if needed
                    if (recv_data[pos] != nullptr) {
                        delete[] recv_data[pos];
                    }
                    recv_data[pos] = new char[temp.size() + 1];
                    memcpy(recv_data[pos], temp.c_str(), temp.size());
                    recv_data[pos][temp.size()] = '\0';
                    //发生在尾部
                  }else if (symbols == 12)
                  {
                    // 从 data_alredy_get[i-node_id[0]] 的起始位置，获取长度为 afer_computer_fail_value_len - recv_len[pos] 的字符串
                    temp = recv_data[pos];
    ...
    // 赋值
    if (afer_computer_fail_value_len == blocksize)
    {
    ...
    { // 优化对齐repair 只有一个错
        ...
        if (value_get_num == 1)
        {
            char *temp = data_ptrs[fail_node[0]];
            value.append(temp);
                                                ...
                                                const std::vector<size_t> &value_lens, std::vector<bool> &request_success)
{
    ...
    for (int i = 0; i < num; i++)
    {
        if (value_lens[i] == 0)
        {
            ...
            get_request_return[i] = 0; // 标记为成功
            request_success[i] = true; // 标记为成功
