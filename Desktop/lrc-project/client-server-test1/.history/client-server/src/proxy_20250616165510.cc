...
void Proxy::ProcessGet_Proxy(std::string value)
{
    ...
    else if (command == "datanode")
    {
        ...
        else
        {
            ...
            size_t max_fail_len = 0;
            int min_offset = offset[0];
            //文件只存在一个datanode中并且该datanode故障
            if (num == 1)
            {
                max_fail_len = value_len_[0];
                min_offset = offset[0];
            }
            else if (fail_num == 1)
            {
                // 如果只有一个节点失败，直接使用该节点的信息
                max_fail_len = value_len_[error_id[0]];
                min_offset = offset[error_id[0]];
            }
            else
            {
                // 多个节点失败，计算最大长度和最小偏移量
            ...
            // for (int i = 0; i < fail_num; i++)
            // {
            //     max_fail_len = std::max(max_fail_len, static_cast<size_t>(value_len_[error_id[i]]));
            //     min_offset = std::min(min_offset, offset[error_id[i]]);
            //     cout<<"max_fail_len: "<<max_fail_len<<" min_offset: "<<min_offset<<endl;
            // }
            ...
            if (repair_message != nullptr && *actual_length > 0)
            {
                ...
                // 执行降级读修复
                Repair_degrade(repair_message, num, node_id, min_offset, max_fail_len,offset, value_len_, value_get_to_client, data_get);
...
void Proxy::resort_recvdata(char **recv_data, size_t *recv_len, int num)
{
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        else
        {
            ...
            fail_find[i] = 1; // 标记为失败
            node_id_index_pairs.emplace_back(-1, i); // 使用-1表示失败的node_id
            continue;
        }
    }
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        if (fail_find[original_index]) {
            // 失败节点置为空数据
            sorted_lengths[i] = 0;
            sorted_data_get[i] = new char[0];
        } else {
    ...
    for (int i = 0; i < num; ++i)
    {
        if (sorted_lengths[i] == 0) continue; // 跳过空数据
...
void Proxy::Repair_degrade(string message, int value_get_num, int *node_id, const int fail_offset, size_t fail_value_len_, const int *offset_value, const size_t *repair_value_len, string &value,const char ** data_alredy_get)
{
    // parse value
    ...
    for (int i = 0; i < num; i++)
    {
        if(fail_value_len_ == 0){
            fail_value_len_ = blocksize; // 如果没有指定失败值长度，使用默认的blocksize
            repair_value_len_vec[i] = fail_value_len_; // 如果没有指定失败值长度，使用默认的blocksize
            offset_value_vec[i] = fail_offset;
        }else{
            offset_value_vec[i] = fail_offset;
            repair_value_len_vec[i] = fail_value_len_;
        }
    ...
    for (int i = 0; i < sum; i++)
    {
        ...
        if (!is_failed && pos < num && i == node_need_id[pos])
        {
            if (i < k && i >= 0)
            {
                data_ptrs[i] = new char[fail_value_len_];
            ...
            {
                global_parity[i - k] = new char[fail_value_len_];
            ...
            {
                local_parity[i - k - r] = new char[fail_value_len_];
    ...
    {
        NewLRC newlrc(k, r, p, blocksize);
        repair_ok = newlrc.muti_decode(fail_number, fail_node, data_ptrs, global_parity, local_parity,fail_value_len_);
    }
    else if (decode_type == "azure")
    {
        AZURE_LRC azure(k, r, p, blocksize);
        repair_ok = azure.muti_decode(fail_number, fail_node, data_ptrs, global_parity, local_parity,fail_value_len_);
    }
    else if (decode_type == "azure_1")
    {
        AZURE_LRC_1 azure_1(k, r, p, blocksize);
        repair_ok = azure_1.muti_decode(fail_number, fail_node, data_ptrs, global_parity, local_parity,fail_value_len_);
    }
    else if (decode_type == "optimal")
    {
        OPTIMAL_LRC optimal(k, r, p, blocksize);
        repair_ok = optimal.muti_decode(fail_number, fail_node, data_ptrs, global_parity, local_parity,fail_value_len_);
    }
    else if (decode_type == "uniform")
    {
        UNIFORM_LRC uniform(k, r, p, blocksize);
        repair_ok = uniform.muti_decode(fail_number, fail_node, data_ptrs, global_parity, local_parity,fail_value_len_);
    }
    else if (decode_type == "sep-uni")
    {
        UNBALANCED_LRC unbalanced(k, r, p, blocksize);
        repair_ok = unbalanced.muti_decode(fail_number, fail_node, data_ptrs, global_parity, local_parity,fail_value_len_);
    }
    else if (decode_type == "sep-azure")
    {
        SEP_AZURE sep_azure(k, r, p, blocksize);
        repair_ok = sep_azure.muti_decode(fail_number, fail_node, data_ptrs, global_parity, local_parity,fail_value_len_);
    }
    // print
    if (repair_ok)
    {
    ...
    }else{ //优化对齐repair 只有一个错
        // 文件位于block中并且该block故障
        if (value_get_num == 1)
        {
            char *temp = data_ptrs[node_id[0]];
            value.append(temp);
...
void Proxy::GetRevData(char **data, int num, size_t *value_len, size_t *&recv_len)
{
    ...
    while (true)
    {
        ...
        recv_thrds.emplace_back([this,num, recv_len, index, conffd_accept, &data, &value_len]()
                                { recv_len[index] = HandleGetRev(num,data[index], conffd_accept, value_len[index]); });
        index++;
        if (index == num)
        {
            break;
...
size_t Proxy::HandleGetRev(int one_or_two,char *&data, int connfd, size_t value_len)
{
