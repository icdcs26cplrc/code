...
class MultiEncodingTester:
    ...
    def simulate_node_failure(self) -> bool:
        ...
        try:
            ...
            # 获取 datanode 进程列表
            datanode_proc = self.component_manager.processes.get("datanode")
            if not datanode_proc or not isinstance(datanode_proc, list):
                print("Datanode 进程列表为空或格式错误")
                return False
            
            # 随机选择一个 datanode 子进程
            target_proc = random.choice(datanode_proc)
            if target_proc.poll() is None:  # 确保进程仍在运行
                print(f"正在终止 datanode 子进程 PID: {target_proc.pid}")
                target_proc.terminate()
                try:
                    target_proc.wait(timeout=3)
                except subprocess.TimeoutExpired:
                    target_proc.kill()
                print(f"Datanode 子进程 PID: {target_proc.pid} 已终止")
            else:
                print(f"Datanode 子进程 PID: {target_proc.pid} 已停止，无需终止")
