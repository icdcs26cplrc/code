...
class MultiEncodingTester:
    ...
    def _create_encoding_strategies(self) -> List[EncodingStrategy]:
        ...
        block_choices = [65536, 1048576, 4194304,10485760]  # 块大小选项 64kb, 1mb, 2mb,10mb
        # 生成所有编码策略
    ...
    def simulate_node_failure(self) -> bool:
        """模拟节点故障 - 根据 datanode 的 PID 直接终止进程"""
        try:
            # 获取 datanode 的日志输出
            datanode_logs = self.component_manager.get_component_output("datanode", timeout=1.0)
            if not datanode_logs:
                print("无法获取 datanode 日志")
                return False
            
            # 从日志中提取 PID
            pid_pattern = r"Created DataNode \d+ \(PID: (\d+), IP: [\d\.]+, Port: \d+\)"
            pids = [int(match.group(1)) for match in re.finditer(pid_pattern, ' '.join(datanode_logs))]
            
            if not pids:
                print("未找到 datanode 的 PID")
                return False
            
            # 随机选择一个 PID
            target_pid = random.choice(pids)
            print(f"正在终止 datanode 进程 PID: {target_pid}")
            
            # 发送 SIGTERM 信号终止进程
            try:
                os.kill(target_pid, signal.SIGTERM)
                time.sleep(2)  # 等待进程终止
                print(f"datanode 进程 PID: {target_pid} 已终止")
            except ProcessLookupError:
                print(f"datanode 进程 PID: {target_pid} 不存在或已终止")
    ...
    def test_single_strategy(self, strategy: EncodingStrategy, num_operations: int = 50) -> Dict:
        ...
        for i in range(num_operations):
            ...
            # 根据比例决定操作类型
            if i < num_operations * 0.7:  # 前70%为SET操作
                op_type = 'SET'
            else :  # 60%-90%为GET操作
                op_type = 'GET'
            # else:  # 最后10%为REPAIR操作
