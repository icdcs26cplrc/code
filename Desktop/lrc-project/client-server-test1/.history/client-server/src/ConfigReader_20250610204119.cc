...
bool ConfigReader::parseXMLConfig(const std::string& configFile) {
    ...
    std::regex attr_regex(R"(<attribute name=\"(\w+)\">)");
    std::regex value_regex(R"(<value>(.*?)</value>)");

    auto attr_iter = std::sregex_iterator(content.begin(), content.end(), attr_regex);
    auto attr_end = std::sregex_iterator();

    while (attr_iter != attr_end) {
        std::smatch attr_match = *attr_iter;
        std::string name = attr_match[1].str();

        std::string value_section = attr_match.suffix().str();
        std::vector<std::string> values;

        auto value_iter = std::sregex_iterator(value_section.begin(), value_section.end(), value_regex);
