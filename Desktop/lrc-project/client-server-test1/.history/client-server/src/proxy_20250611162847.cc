...
void Proxy::ProcessGet_Proxy(std::string value)
{
    ...
    else if (command == "datanode")
    {
        ...
        ProcessGetRequests(node_ids, offset_vec, value_len_vec);
...
// 修改后的GET请求处理，添加线程隔离和错误处理
    ...
    // 修改后的GET请求处理函数
    void Proxy::ProcessGetRequests(const std::vector<std::string>& node_ids, const std::vector<int>& offsets, const std::vector<size_t>& value_lens) {
    ...
    int Proxy::SendGetRequestWithTimeout(const std::string& node_id, int offset, size_t value_len, std::chrono::seconds timeout) {
        ...
        try {
            // 创建连接
            int sock = CreateConnectionWithTimeout(timeout);
            if (sock < 0) {
    ...
    // 创建带超时的连接
    int Proxy::CreateConnectionWithTimeout(std::chrono::seconds timeout) {
        int sock = socket(AF_INET, SOCK_STREAM, 0);
        if (sock < 0) {
            return -1;
        }
        ...
        sockaddr_in server_addr{};
        server_addr.sin_family = AF_INET;
        server_addr.sin_port = htons(DATANODE_PORT_BASE); // 根据实际情况修改
    ...
    void Proxy::GetRevDataWithIsolation(char** data, int num, const size_t* value_lens, size_t*& recv_len, const int* request_status) {
        ...
        for (int i = 0; i < num; i++) {
            if (request_status[i] == 0) { // 只计算请求成功的
