...
void Proxy::Repair_degrade(string message, int value_get_num, int *node_id, int *offset_value, size_t *repair_value_len, string &value)
{
    ...
    std::vector<int> offset_value_vec(offset_value, offset_value + num);
                                                ...
                                                const std::vector<size_t> &value_lens, std::vector<bool> &request_success)
{
    ...
    if (successful_requests > 0)
    {
        ...
        GetRevDataWithErrorHandling(data_get, num, value_lens.data(), recv_len, request_success);
...
// 带错误处理的数据接收函数
void Proxy::GetRevDataWithErrorHandling(char **data, int num, const size_t *value_lens,
                                        size_t *&recv_len, const std::vector<bool> &request_success)
{
    ...
    // 计算预期的成功连接数
    int expected_connections = 0;
    int* request_status = new int[num];
    for (int i = 0; i < num; i++)
    {
        if (request_success[i])
        {
            request_status[i] = 0; // 请求成功
            expected_connections++;
        }else if (!request_success[i])
        {
            request_status[i] = -1; // 请求失败
        }
    ...
    {
        recv_threads.emplace_back([this, data, recv_len, value_lens, num,
                                   &connections_handled, &accept_mutex, &request_success,request_status]()
                                  {
try {
int connfd = -1;
...
if (node_index >= 0 && node_index < num) {
cout << "Receiving data for node index " << node_index << endl;
recv_len[node_index] = HandleGetRevWithIsolation(data[node_index], connfd, 
                                  value_lens[node_index], node_index);
connections_handled++;
} else {
cerr << "Could not find available node index for connection" << endl;
}
